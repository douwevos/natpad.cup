...
#define CAT_LOG_LEVEL CAT_LOG_[%debug.level%]
#define CAT_LOG_CLAZZ "[%Pre%]ParserBase"
#include <logging/catlog.h>

G_DEFINE_TYPE([%Pre%]ParserBase, [%pre%]_parser_base, G_TYPE_OBJECT)

static void _dispose(GObject *object);

static void [%pre%]_parser_base_class_init([%Pre%]ParserBaseClass *clazz) {
	GObjectClass *object_class = G_OBJECT_CLASS(clazz);
	object_class->dispose = _dispose;
}

static void [%pre%]_parser_base_init([%Pre%]ParserBase *obj) {
}
...
[%Pre%]ParserBase *[%pre%]_parser_base_new() {
	[%Pre%]ParserBase *result = g_object_new([%PRE%]_TYPE_PARSER_BASE, NULL);
...
[%Pre%]Token *[%pre%]_parser_base_parse([%Pre%]ParserBase *parser_base, [%Pre%]ParserContext *parserContext) {
	...
	const [%Pre%]ParserBaseClass *base_class = [%PRE%]_PARSER_BASE_GET_CLASS(parser_base);
	const [%Pre%]ParserContextClass *parser_context_class = [%PRE%]_PARSER_CONTEXT_GET_CLASS(parserContext);
	...
	cat_array_wo_clear(parserContext->e_stack);
	
	
	[%Pre%]IScannerInterface *scanner_iface = [%PRE%]_ISCANNER_GET_INTERFACE(parserContext->scanner);
	
	[%Pre%]Token *start_token = scanner_iface->createToken(parserContext->scanner, 0, base_class->start_state(parser_base), FALSE, FALSE, FALSE, 0, 0, 0, 0, NULL);
	parser_context_class->push(parserContext, start_token);	
	cat_unref_ptr(start_token);
//	parser_context_class->push(parserContext, [%pre%]_token_new_symbol_state(0, base_class->start_state(parser_base)));
	parserContext->tos = 0;
...
//			throw new Error("Symbol recycling detected (fix your scanner).");
		...
		[%pre%]_token_dump(parser_context_class->peek(parserContext));
		...
		act = [%pre%]_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, parserContext->current_token->sym);
		...
		if (act > 0) {
			...
			cat_log_debug("lhs_sym %d[%if-set:symbol.info%], %s[%end-if:symbol.info%]", lhs_sym->sym[%if-set:symbol.info%], lhs_sym->symbol_text[%end-if:symbol.info%]);
			...
			act = [%pre%]_parser_base_get_reduce(parser_base, parser_context_class->peek_at(parserContext, handle_size)->parse_state, lhs_sym_num);
...
//			act = [%pre%]_parser_base_get_reduce(parser_base, parser_context_class->peek(parserContext)->parse_state, lhs_sym_num);
//
//			/* shift to that state */
//			lhs_sym->parse_state = act;
//			lhs_sym->used_by_parser = TRUE;
...
//			[%pre%]_parser_report_syntax_error(parserContext->current_token);
			...
			if (![%pre%]_parser_base_error_recovery(parser_base, parserContext, FALSE)) {
...
//		if (debug) cat_log_debug("# Error recovery fails");
	...
	while(TRUE) {
		...
		[%Pre%]Token *bottomToken = ([%Pre%]Token *) cat_array_wo_get(parserContext->e_lookahead, 0);
		if (bottomToken==NULL) {
...
//		cat_log_debug("# Consuming Symbol #" + parserContext->e_lookahead[0].sym);
...
gboolean [%pre%]_parser_base_shift_under_error([%Pre%]ParserBase *parser_base, [%Pre%]ParserContext *parserContext) {
	...
	[%Pre%]ParserContextClass *parser_context_class = [%PRE%]_PARSER_CONTEXT_GET_CLASS(parserContext);
	return [%pre%]_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, errorSym) > 0;
}
...
//		if (debug) cat_log_debug("# Pop stack by one, state was # " + parser_context_class->peek(parserContext).parse_state);
		...
		if (cat_array_wo_size(parserContext->e_stack)==0) {
	...
	act = [%pre%]_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, base_class->error_symbol(parser_base));
//	if (debug) {
...
//	error_token = [%pre%]_token_new_full(base_class->error_symbol(parser_base), FALSE, left_pos, left_row, right_pos, right_row, NULL);[%if-set:symbol.info%]
//	error_token->symbol_text= "error"; [%end-if:symbol.info%]
...
void [%pre%]_parser_base_read_lookahead([%Pre%]ParserBase *parser_base, [%Pre%]ParserContext *parserContext) {
	...
	if (parserContext->e_lookahead==NULL) {
		parserContext->e_lookahead = cat_array_wo_new();
//		parserContext.lookahead = new [%Pre%]Token[[%pre%]_parser_error_sync_size()];
	} else {
		cat_array_wo_clear(parserContext->e_lookahead);
	}
	...
	for(i = 0; i < parser_base->error_sync_size; i++) {
		cat_array_wo_append(parserContext->e_lookahead, (GObject *) parserContext->current_token);
...
void [%pre%]_parser_base_restart_lookahead([%Pre%]ParserBase *parser_base, [%Pre%]ParserContext *parserContext) {
	...
	for(i = 1; i < parser_base->error_sync_size; i++) {
		cat_array_wo_set(parserContext->e_lookahead, cat_array_wo_get(parserContext->e_lookahead, i), i-1, NULL);
	}
	...
	cat_array_wo_set(parserContext->e_lookahead, (GObject *) (parserContext->current_token), parser_base->error_sync_size-1, NULL);
...
gboolean [%pre%]_parser_base_try_parse_ahead([%Pre%]ParserBase *parser_base, [%Pre%]ParserContext *parserContext, gboolean debug) {
	...
	[%Pre%]VirtualParseStack *vstack = [%pre%]_virtual_parse_stack_new(parserContext->e_stack);
	...
	while(TRUE) {
		...
		if (act > 0) {
			...
			[%Pre%]Token *token2push = scanner_iface->createToken(parserContext->scanner, 0, act-1, FALSE, FALSE, FALSE, 0, 0, 0, 0, NULL);
			[%pre%]_virtual_parser_stack_push(vstack, token2push);
			cat_unref_ptr(token2push);
			
//			[%pre%]_virtual_parser_stack_push(vstack, [%pre%]_token_new_symbol_state(0, act-1));

//			if (debug) cat_log_debug("# Parse-ahead shifts Symbol #" + [%pre%]_parser_context_current_error_token(parserContext).sym + " into state #" + (act - 1));
...
//			if (debug) cat_log_debug("# Parse-ahead reduces: handle size = " + rhs_size + " lhs = #" + lhs + " from state #" + vstack.[%pre%]_parser_top());
			...
			[%Pre%]Token *token2push = scanner_iface->createToken(parserContext->scanner, 0, reduced_state, FALSE, FALSE, FALSE, 0, 0, 0, 0, NULL);
			[%pre%]_virtual_parser_stack_push(vstack, token2push);
			cat_unref_ptr(token2push);
//			[%pre%]_virtual_parser_stack_push(vstack, [%pre%]_token_new_symbol_state(0, reduced_state));
//			if (debug) cat_log_debug("# Goto state #" + vstack.[%pre%]_parser_top());
		}
	}
...
//		cat_log_debug("# Current Symbol is #" + [%pre%]_parser_context_current_error_token(parserContext).sym);
...
//	}
	...
	while (!parserContext->done_parsing) {
		...
		act = [%pre%]_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, error_token->sym);
...
//			if (debug) cat_log_debug("# Current Symbol is #" + [%pre%]_parser_context_current_error_token(parserContext).sym);
		}
		/* if its less than zero, then it encodes a reduce action */
		else if (act < 0) {
			/* perform the action for the reduce */
