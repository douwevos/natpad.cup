 ...
 *  </dl>
 *
 * @see     net.natpad.cup.model.parser.generated.runtime.LrScanner
 * @see     net.natpad.cup.model.parser.generated.runtime.LrScanner
 * @see     org.natpad.cup.runtime.virtual_parse_stack
...
public abstract class LrParser {
	 ...
	 */
	public abstract short[][] getProductionTable();
	 ...
	 * 
	 * @see org.natpad.cup.simplecalc.runtime.LrParser#get_action
	 */
	public abstract short[][] getActionTable();
	 ...
	 * 
	 * @see org.natpad.cup.simplecalc.runtime.LrParser#get_reduce
	 */
	public abstract short[][] getReduceTable();
	 ...
	 * subclass).
	 */
	public abstract int eofSymbol();

	/* . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . */

	/** The index of the special error Symbol (supplied by generated subclass). */
	public abstract int errorSymbol();
...
//	 * private. [CSA/davidm]
//	 */
//	private LrScanner _scanner;
//
//	/**
//	 * Simple accessor method to set the default scanner.
//	 */
//	public void setScanner(LrScanner s) {
//		_scanner = s;
//	}
//
//	/**
...
//	}
	 ...
	 */
	public abstract LrSymbol runAction(LrParserContext parserContext, int actionId) throws java.lang.Exception;
	 ...
	 * body.
	 */
	public void userInit() throws java.lang.Exception {
	}
	 ...
	 */
	protected abstract void initActions() throws java.lang.Exception;
	 ...
	 */
	public LrSymbol scan(LrParserContext parserContext) throws java.lang.Exception {
		LrSymbol sym = parserContext.scanner.next_token();
		return (sym != null) ? sym : new LrSymbol(eofSymbol());
	}
	 ...
	 */
	public void report_fatal_error(LrParserContext parserContext, String message, Object info) throws java.lang.Exception {
	 ...
	 */
	public void reportSyntaxError(LrSymbol cur_token) {
	 ...
	 */
	public void unrecovered_syntax_error(LrParserContext parserContext, LrSymbol cur_token) throws java.lang.Exception {
		report_fatal_error(parserContext, "Couldn't repair and continue parse", cur_token);
	}
	 ...
	 */
	public LrSymbol parse(LrParserContext parserContext) throws java.lang.Exception {
		/* the current action code */
		int act;

		/* the Symbol/stack element returned by a reduce */
		LrSymbol lhs_sym = null;
		...
		production_tab = getProductionTable();
		action_tab = getActionTable();
		reduce_tab = getReduceTable();
		...
		/* get the first token */
		parserContext.cur_token = scan(parserContext);

		/* push dummy Symbol with start state to get us underway */
		parserContext.stack.removeAllElements();
		parserContext.push(new LrSymbol(0, startState()));
		parserContext.tos = 0;

		/* continue until we are told to stop */
		for (parserContext._done_parsing = false; !parserContext._done_parsing;) {
...
//			parserContext.dump();
			...
			act = getAction(parserContext.peek().parse_state, parserContext.cur_token.sym);

//			System.out.println("state="+parserContext.peek().parse_state+", sym="+parserContext.cur_token.sym+", act="+act);

			
			/* decode the action -- > 0 encodes shift */
			if (act > 0) {
				/* shift to the encoded state by pushing it on the stack */
				parserContext.cur_token.parse_state = act - 1;
				parserContext.cur_token.used_by_parser = true;
				parserContext.shift(parserContext.cur_token);
...
//				System.out.println("running action "+ ((-act) - 1));
				lhs_sym = runAction(parserContext, (-act) - 1);

//				System.out.println("lhs_sym " +lhs_sym.sym);
				...
				act = getReduce(parserContext.peek().parse_state, lhs_sym_num);

//				System.out.println("lhs_sym_num="+lhs_sym_num+", handle_size="+handle_size+", reduced-parse-state="+act);

				
				/* shift to that state */
				lhs_sym.parse_state = act;
				lhs_sym.used_by_parser = true;
				parserContext.shift(lhs_sym);
			}
			/* finally if the entry is zero, we have an error */
			else if (act == 0) {
				/* call user syntax error reporting routine */
				reportSyntaxError(parserContext.cur_token);

				/* try to error recover */
				if (!error_recovery(parserContext, false)) {
					/* if that fails give up with a fatal syntax error */
					unrecovered_syntax_error(parserContext, parserContext.cur_token);

					/* just in case that wasn't fatal enough, end parse */
					parserContext.doneParsing();
				} else {
					lhs_sym = parserContext.peek();
				}
			}
		}
		return lhs_sym;
	...
	public void dump_stack(LrParserContext parserContext) {
		if (parserContext.stack == null) {
		...
		/* dump the stack */
		for (int i = 0; i < parserContext.stack.size(); i++) {
			debug_message("LrSymbol: " + ((LrSymbol) parserContext.stack.elementAt(i)).sym + " State: "
							+ ((LrSymbol) parserContext.stack.elementAt(i)).parse_state);
		}
		debug_message("==========================================");
	}
	 ...
	 */
	public void debug_shift(LrSymbol shift_tkn) {
	 ...
	 */
	public void debug_stack(LrParserContext parserContext) {
		StringBuffer sb = new StringBuffer("## STACK:");
		for (int i = 0; i < parserContext.stack.size(); i++) {
			LrSymbol s = (LrSymbol) parserContext.stack.elementAt(i);
			sb.append(" <state " + s.parse_state + ", sym " + s.sym + ">");
			if ((i % 3) == 2 || (i == (parserContext.stack.size() - 1))) {
	 ...
	 * debugging messages.
	 */
	public LrSymbol debug_parse(LrParserContext parserContext) throws java.lang.Exception {
		/* the current action code */
		int act;
		...
		debug_message("# Current Symbol is #" + parserContext.cur_token.sym);
		...
		for (parserContext._done_parsing = false; !parserContext._done_parsing;) {
			...
			if (act > 0) {
				...
				parserContext.cur_token.parse_state = act - 1;
				parserContext.cur_token.used_by_parser = true;
				debug_shift(parserContext.cur_token);
				parserContext.shift(parserContext.cur_token);

				/* advance to the next Symbol */
				parserContext.cur_token = scan(parserContext);
				debug_message("# Current token is " + parserContext.cur_token);
			}
			/* if its less than zero, then it encodes a reduce action */
			else if (act < 0) {
				/* perform the action for the reduce */
				...
				/* look up the state to go to from the one popped back to */
				act = getReduce(parserContext.peek().parse_state, lhs_sym_num);
				debug_message("# Reduce rule: top state " + parserContext.peek().parse_state + ", lhs sym "
			...
			/* finally if the entry is zero, we have an error */
				...
				/* try to error recover */
				if (!error_recovery(parserContext, true)) {
					/* if that fails give up with a fatal syntax error */
					unrecovered_syntax_error(parserContext, parserContext.cur_token);
	 ...
	 */
	protected boolean error_recovery(LrParserContext parserContext, boolean debug) throws java.lang.Exception {
		if (debug)
		 ...
		 */
		if (!find_recovery_config(parserContext, debug)) {
			if (debug)
		...
		for (;;) {
			...
			if (tryParseAhead(parserContext, debug)) {
				break;
			}

			/* if we are now at EOF, we have failed */
			if (parserContext.lookahead[0].sym == eofSymbol()) {
				if (debug)
			...
			if (debug) debug_message("# Consuming Symbol #" + parserContext.lookahead[0].sym);
			restart_lookahead(parserContext);
		}
		...
		parse_lookahead(parserContext, debug);
	 ...
	 */
	protected boolean shift_under_error(LrParserContext parserContext) {
		/* is there a shift under error Symbol */
		return getAction(parserContext.peek().parse_state, errorSymbol()) > 0;
	}
	 ...
	 */
	protected boolean find_recovery_config(LrParserContext parserContext, boolean debug) {
		LrSymbol error_token;
		int act;

		if (debug) debug_message("# Finding recovery state on stack");

		/* Remember the right-position of the top symbol on the stack */
		int right_pos = parserContext.peek().right;
		int left_pos = parserContext.peek().left;

		/* pop down until we can shift under error Symbol */
		while (!shift_under_error(parserContext)) {
			/* pop the stack */
			if (debug)
				debug_message("# Pop stack by one, state was # " + parserContext.peek().parse_state);
			left_pos = parserContext.pop().left;
		...
		act = getAction(parserContext.peek().parse_state, errorSymbol());
		if (debug) {
			debug_message("# Recover state found (#" + parserContext.peek().parse_state + ")");
			debug_message("# Shifting on error to state #" + (act - 1));
		}

		/* build and shift a special error Symbol */
		error_token = new LrSymbol(errorSymbol(), left_pos, right_pos);
		error_token.parse_state = act - 1;
		error_token.used_by_parser = true;
		parserContext.shift(error_token);
	 ...
	 * Symbols.
	 */
	protected void read_lookahead(LrParserContext parserContext) throws java.lang.Exception {
		/* create the lookahead array */
		parserContext.lookahead = new LrSymbol[error_sync_size()];

		/* fill in the array */
		for (int i = 0; i < error_sync_size(); i++) {
			parserContext.lookahead[i] = parserContext.cur_token;
			parserContext.cur_token = scan(parserContext);
		}
	...
	protected LrSymbol currentErrorToken(LrParserContext parserContext) {
		return parserContext.lookahead[parserContext.lookahead_pos];
	}
	 ...
	 */
	protected boolean advance_lookahead(LrParserContext parserContext) {
		/* advance the input location */
		parserContext.lookahead_pos++;

		/* return true if we didn't go off the end */
		return parserContext.lookahead_pos < error_sync_size();
	}
	 ...
	 */
	protected void restart_lookahead(LrParserContext parserContext) throws java.lang.Exception {
		/* move all the existing input over */
		for (int i = 1; i < error_sync_size(); i++)
			parserContext.lookahead[i - 1] = parserContext.lookahead[i];
		...
		parserContext.lookahead[error_sync_size() - 1] = parserContext.cur_token;
		parserContext.cur_token = scan(parserContext);
	 ...
	 * @param debug   should we produce debugging messages as we parse.
	 */
	protected boolean tryParseAhead(LrParserContext parserContext, boolean debug) throws java.lang.Exception {
		/* create a virtual stack from the real parse stack */
		LrVirtualParseStack vstack = new LrVirtualParseStack(parserContext.stack);

		/* parse until we fail or get past the lookahead input */
		while(true) {
			/* look up the action from the current state (on top of stack) */
			int act = getAction(vstack.top(), currentErrorToken(parserContext).sym);
			...
			if (act > 0) {
				...
				if (debug) debug_message("# Parse-ahead shifts Symbol #" + currentErrorToken(parserContext).sym + " into state #" + (act - 1));

				/* advance simulated input, if we run off the end, we are done */
				if (!advance_lookahead(parserContext))
					return true;
			} else {
			/* < 0 encodes a reduce */
				...
				vstack.push(getReduce(vstack.top(), lhs));
	 ...
	 *            should we produce debugging messages as we parse.
	 */
	protected void parse_lookahead(LrParserContext parserContext, boolean debug) throws java.lang.Exception {
		/* the current action code */
		int act;
		...
		if (debug) {
			...
			debug_message("# Current Symbol is #" + currentErrorToken(parserContext).sym);
			debug_message("# Current state is #" + parserContext.peek().parse_state);
		}

		/* continue until we accept or have read all lookahead input */
		while (!parserContext._done_parsing) {
			/* current state is always on the top of the stack */

			/* look up action out of the current state with the current input */
			act = getAction(parserContext.peek().parse_state, currentErrorToken(parserContext).sym);

			/* decode the action -- > 0 encodes shift */
			if (act > 0) {
				/* shift to the encoded state by pushing it on the stack */
				currentErrorToken(parserContext).parse_state = act - 1;
				currentErrorToken(parserContext).used_by_parser = true;
				if (debug)
					debug_shift(currentErrorToken(parserContext));
				parserContext.shift(currentErrorToken(parserContext));
			 ...
			 */
				...
				report_fatal_error(parserContext, "Syntax error", lhs_sym);
				return;
			}
		}
